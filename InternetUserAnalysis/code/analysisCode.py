...
def global_internet_users_analysis():
    ...
    plt.savefig('../chartOutput/全球用户每年的各项数据的分析与可视化.png')
    plt.show()
...
def entities_2020_internet_users_percentage_pie_bar():
    ...
    plt.savefig('../chartOutput/2020年各个国家地区的互联网用户占比饼图.png')
    plt.show()
    ...
    plt.savefig('../chartOutput/2020年各个国家地区的互联网用户占比柱状图.png')
    plt.show()
...
def entities_2020_internet_users_percentage_distribution_histogram():
    ...
    plt.savefig('../chartOutput/2020年各国家地区互联网用户占比分布直方图.png')
    plt.show()
...
def entities_2020_internet_users_percentage_distribution_scatter():
    ...
    plt.savefig('../chartOutput/2020年个国家地区互联网用户占比和移动互联网订阅量散点图及线性回归拟合.png')
    plt.show()
...
def draw_internet_users_percentage_annual_top_3_wordcloud():
    ...
    plt.imshow(wc)
    plt.axis("off")
    wc.to_file('../chartOutput/每年互联网用户的比例最大的国家地区名词云.png')
    plt.show()
...
def chinese_users_analysis():
    ...
    plt.savefig('../chartOutput/中国互联网用户的数量（单位：千万人）、占人口的比例、移动互联网订阅每一百人比例、宽带每一百人订阅比例.png')
    plt.show()
    ...
    sns.lineplot(data=chinese_users, x='Year', y='increase of Broadband Subscription', lw=4,
                 label='宽带每一百人订阅比例增长率')
    plt.legend(loc='upper left')
    plt.savefig('../chartOutput/中国互联网用户的数量（单位：千万人）、占人口的比例、移动互联网订阅每一百人比例、宽带每一百人订阅比例的增长率.png')
    plt.show()
    ...
    sns.lineplot(data=data, x='x', y='pred_y')
    plt.savefig('../chartOutput/对1980到2020年中国互联网总用户数的拟合')
    plt.show()
    ...
    plt.savefig('../chartOutput/到2030年中国互联网总用户数的预测')
    plt.show()


if __name__ == '__main__':
    # 读取文件，获取全球互联网用户信息
    global_users = pd.read_csv('../Dataset/Final.csv', delimiter=',', usecols=range(1, 8))  # 由于第一列的列名未知，所以不使用第一列
    # 对全球用户进行分析：
